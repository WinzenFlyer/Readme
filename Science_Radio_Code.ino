...
float DewPoint(){
  ...
  float H = (log(RH) - 2)/0.4343 + (17.62*T)/(243.12 + T);
  float delta = 243.12*H/(17.62-H);
...
void setup(){
  sprintf(WAITSTRING, "System Starting, please wait...\n");
...
void loop()
{
  ...
  fmtDouble(alt, 2, AltitudeString);
  fmtDouble(temperatureDS, 2, DSTemperString);
  fmtDouble(hum, 2, HumidityString);
  fmtDouble(delta, 2, DewPointString);
  fmtDouble(volt, 2, VoltageString);
  
  sprintf(DATASTRING, "$$OERNEN-II,%d,T = %s, P = %s, A = %s, Td = %s, H = %s, Dp = %s, V = %s\n", count, TemperatureString, PressureString, AltitudeString, DSTemperString, HumidityString, DewPointString, VoltageString);
